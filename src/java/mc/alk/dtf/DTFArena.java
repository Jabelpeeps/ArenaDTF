...
public class DTFArena extends Arena {
	...
	final Map<Integer, Location> returnPoints = new ConcurrentHashMap<Integer, Location>();
	...
	public void onStart(){
		...
		if (flagSpawns.size() < 2){
			Log.err("Cancelling DTF as there aren't 2 flags");
		...
		for (Location l: flagSpawns.values()){
			...
			Flag f = i > 0 ? new CapturePoint(t,is,l) : new Flag(t,is,l);
			teamFlags.put(t, f);
	...
	public void onPlayerPickupItem(PlayerPickupItemEvent event){
		...
		if (flag.team.equals(t)){
			...
			if (!flag.isHome()){  /// Return the flag back to its home location
				...
				spawnFlag(flag);
				t.sendMessage(mmh.getMessage("DefendTheFlag.player_returned_flag", params));
			}
		} else {
			...
			for (ArenaTeam team : getTeams()){
				if (team.equals(t)){
					team.sendMessage(mmh.getMessage("DefendTheFlag.taken_enemy_flag", params));
				} else if (team.equals(fteam)){
					team.sendMessage(mmh.getMessage("DefendTheFlag.taken_your_flag", params));
				}
			}
		}
	}
	...
	public void onPlayerMove(PlayerMoveEvent event){
		...
		if (f.isHome() && nearLocation(f.getCurrentLocation(),event.getTo())){
			...
			mmh.sendMessage("DefendTheFlag.teamscored",params);
	...
	public List<String> getInvalidReasons(){
		...
		if (flagSpawns == null || flagSpawns.size() < 2){
			reasons.add("You need to set 2 at least 2 flags!");}
